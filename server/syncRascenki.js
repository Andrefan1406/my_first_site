...
// Текст для эмбеддинга: наименование работы и единица ПЕРВЫМИ, раздел/подраздел
// после — как уточняющий контекст. Раньше было наоборот (раздел вперёд), но
// длинные названия разделов «размывали» вектор: по короткому запросу
// («наливной пол») строки с этим же названием работы недобирали косинус
// из-за веса слов раздела. Порядок «работа → контекст» релевантность заметно
// поднимает.
function buildSearchableText(row) {
  return [row.name, row.unit, row.section, row.subsection].filter(Boolean).join(' ');
}
