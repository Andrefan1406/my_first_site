...
//   1. Расценки утверждённые в ТОО VK development group — все объекты
//   2. Расценки из ЕНиР для бригад
//   3. Расценки из ЕНиР для фирм
//   4. Расценки утверждённые в ТОО VK development group — коммерция (object != «Все объекты»)
...
// Название класса в своде переименовали ("на предприятии" → "в ТОО VK
// development group"), но старую строку в коде не поправили — из-за этого
// фильтр по class не совпадал НИ С ОДНОЙ из 135 строк этой категории, и оба
// блока "утверждённые" всегда были пустыми, независимо от запроса.
const APPROVED = 'Расценки утверждённые в ТОО VK development group';
const ALL_OBJECTS = 'Все объекты';

// Блоки квадранта: блоки 1 и 4 — одна категория («утверждённые в ТОО VK
// development group»), различаются только полем object, поэтому фильтры
// строим через must/must_not по object. Столбец «Категория» их не различает
