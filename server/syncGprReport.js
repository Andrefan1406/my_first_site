...
const SOURCES = [
  {
    key: 'poz64_72',
    label: 'ГПР 64,72,59,63,65,69 и Экополис 103,104,105',
    // Три листа под одним источником — один и тот же ответственный
    // человек за все эти позиции (см. переписку: объединили "64,72" с
    // "59,63,65,69" — тот же объект "Нурлы Жол 3", другая часть позиций
    // с того же участка Макажанова — и отдельно с "ГПР Экополис
    // поз.103,104,105", ранее самостоятельным источником 'ekopolis2').
    sheets: [
      ...
      { spreadsheetId: '1kpCz-ltR4JLqy_IzEYHTvypq_DbnFRllZzfc3ck3fC8', sheetName: 'Факт' },
    ],
    ...
    // не просили. Остальные позиции (59,63,65,69,103,104,105) — с других
    // двух листов, там нет "коммерческих" вариантов, включаем все.
    includePosition: (pos) =>
      ['поз.64', 'поз.72', 'поз.59', 'поз.63', 'поз.65', 'поз.69', 'поз.103', 'поз.104', 'поз.105'].includes(pos),
    // Первые три — строки-разделы без данных на листе "64,72" (заголовок
    // группы работ, а не отдельная работа). Остальные — с листа "Факт"
    // (Экополис): "...Предоплата"/"Плита АКП, м2"/"Утеплитель, м3" — не %
    // готовности работы, а разовая оплата/материал: явно записаны нулём
    // на несколько недель подряд, потом уходят в пустоту — под обычную
    // логику пропуска это ложно засчиталось бы как "забыли внести", хотя
    // это не так.
    excludeWorkNames: new Set([
      'Кровельные работы', 'Окна и витражи', 'ВК и ОВ',
