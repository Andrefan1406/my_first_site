...
// (тот же источник, что читает src/ConcreteDailyReportPage.js на клиенте) ->
// таблица concrete_orders. Полная перезапись при каждом синке: у строк
// исходной таблицы нет стабильного ID, поэтому upsert не построить надёжно,
// а объём данных небольшой — DELETE+INSERT в одной транзакции атомарны для
// читателей.
const cron = require('node-cron');
const Papa = require('papaparse');
const { getWriteDb } = require('./db');

// Все заявки собраны в один лист (gid=0) той же книги — раньше клиент читал
// gid=949231644 (частичный лист), теперь читаем сводный лист.
const CSV_URL =
...
// "Позиция". Склеиваем обратно в одну строку.
...
function normalizeRow(row) {
  return {
    ...
    // что реально выбирает заявитель в форме; вторая, с тем же названием,
    // дальше по листу — не категория вовсе, а разрозненные логистические
    // пометки, PapaParse переименовывает её в "Категория_1". Берём только
    // первую, вторую игнорируем полностью.
