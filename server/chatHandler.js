...
только сводный total_headcount за день):
  ...
  total_headcount  REAL    -- сумма headcount по участку за день (NULL, если данных нет: 'missing' и 'resolved_no_report')
  entries_count    INTEGER -- сколько строк отчёта было в этот день
  decided_by       TEXT    -- email администратора для resolved_copy/resolved_no_report, иначе NULL
...
- Отсутствие отчёта в ЛЮБОЙ календарный день (включая субботу и воскресенье) означает пропуск,
  требующий решения человека. Такие дни системой АВТОМАТИЧЕСКИ НЕ ЗАПОЛНЯЮТСЯ — они остаются со
  статусом 'missing', пока администратор явно не примет решение через отдельную админ-панель
  (посмотреть пропуск и решить: скопировать данные с другого дня или подтвердить, что участок не
  работал). Пока решения нет, в people_reports_resolved для этого дня НЕТ строк вообще (данные
  отсутствуют, а не придуманы). Раньше выходной без отчёта считался нормой и пропуском не был — это
  правило убрано по требованию пользователя: теперь выходные обрабатываются так же, как рабочие дни.
- Для "процента заполненности отчётности" по участку считай:
  COUNT(status='real') * 100.0 / COUNT(status IN ('real','missing','resolved_copy','resolved_no_report'))
  — знаменатель включает ВСЕ дни периода (рабочие и выходные), исключений больше нет.
...
- Для "сколько пропусков заполнено/рассмотрено администратором" -> status IN ('resolved_copy','resolved_no_report').
- Для "количества людей"/"численности"/"динамики численности" используй people_reports_resolved
  ...
  администратором данные, а НЕ сырую таблицу people_reports и не 'missing' дни.
...
- Вопросы про "заполнено вручную"/"решено администратором" -> status IN ('resolved_copy','resolved_no_report').
