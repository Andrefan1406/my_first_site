...
// email из списка people_gap_check_rules блокируется заявка на технику и
// заявка на бетон/раствор — только их обычные роуты /request и
// /concrete-request (GprReportGuard/PeopleGapsGuard в App.js), пока по
// закреплённому за ним участку есть незакрытый пропуск; остальные типы
// заявок не ограничены. «Умная заявка» (SmartRequestPage.jsx) НАМЕРЕННО не
// делает эту проверку сама — оставлена как есть, чтобы у заблокированных
// пользователей был стимул пробовать и привыкать к этому инструменту, а не
// упираться в стену сразу) и правила проверки
  ...
  return (
    <div style={s.page}>
      ...
      <section style={s.section}>
        ...
        <p style={s.hint}>
          Для email из списка ниже блокируется подача заявки на технику и заявки на бетон/раствор
          через обычные формы — пока по закреплённому за ним участку есть незакрытый пропуск
          отчётности (кроме сегодняшнего дня). Остальные типы заявок (электрики, геодезисты,
          лаборатория, брусчатка, ЖБИ), а также «Умная заявка» этой проверкой не ограничены —
          «Умная заявка» специально оставлена доступной, чтобы приучать к ней пользователей.
        </p>
      ...
      <section style={{ ...s.section, marginTop: "20px" }}>
        ...
        <p style={s.hint}>
          Для email из списка блокируется подача заявки на технику и заявки на бетон/раствор через
          обычные формы, пока в соответствующем графике производства работ есть незаполненный %
          готовности за прошлую пятницу или раньше. Как только отчёт заполнят — блокировка снимается
          сама. У каждого источника ГПР свой отдельный список email — за них отвечают разные люди.
          Остальные типы заявок, а также «Умная заявка» этой проверкой не ограничены — «Умная заявка»
          специально оставлена доступной, чтобы приучать к ней пользователей.
        </p>
      </section>
