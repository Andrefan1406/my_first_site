...
const PeopleReportCharts = () => {
  ...
  useEffect(() => {
    fetch('https://docs.google.com/spreadsheets/d/e/2PACX-1vS0qVYHkI9ySfT0LO9SwG36BYrmI-chO09ws7GSjWcnQU2pX4Gzw-R4LXg6tdi44KXa1i5yQYcLF27U/pub?output=csv')
      ...
      .then(text => {
        ...
        const allProfs = [...profSet];
        setProfessions(allProfs);
        if (selectedProfessions.length === 0) setSelectedProfessions(allProfs);
        ...
        dataRows.forEach(row => {
          ...
          const count = parseFloat(row['Количество']) || 0;
          const dateObj = new Date(dateStr);
          if (dateObj.getFullYear() === 2025 && dateStr !== todayStr) {
            const key = dateStr.slice(0, 10);
            dailyTotals[key] = (dailyTotals[key] || 0) + count;
          }
        });
        ...
        // Сравнение 2024 vs 2025 по выбранным профессиям
        ...
        dataRows.forEach(row => {
          ...
          if (dayKey === todayStr || holidays.has(dayKey)) return;
        ...
        Object.entries(dailyTotalsByYear).forEach(([year, dayMap]) => {
          Object.entries(dayMap).forEach(([day, total]) => {
            ...
            if (!monthlySums[year][monthKey]) monthlySums[year][monthKey] = { sum: 0, count: 0 };
  ...
  return (
    <div style={{ width: '100%', height: '100%', padding: '20px', boxSizing: 'border-box' }}>
      ...
      <h2>Сравнение по месяцам: 2024 vs 2025 (по выбранным профессиям)</h2>
      <label>
        Фильтр по профессиям:<br />
        <select multiple size="6" value={selectedProfessions} onChange={(e) => {
